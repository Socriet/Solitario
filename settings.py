...
class Settings:
    def __init__(
        self, waste_size=3, deck_passes_allowed=1000, card_back="/images/card_back0.png", table_background=ft.Colors.GREEN_900, best_score=0, best_time=float('inf'), least_moves=float('inf'), board_scale=1.0
    ):
...
class SettingsDialog(ft.AlertDialog):
    def __init__(self, settings, on_settings_applied):
        ...
        self.waste_size = ft.RadioGroup(
            ...
            content=ft.Row(
                controls=[
                    ft.Radio(value=1, label="1 Card"),
                    ft.Radio(value=3, label="3 Cards"),
                ],
                wrap=True
            ),
        )
        self.deck_passes_allowed = ft.RadioGroup(
            ...
            content=ft.Row(
                controls=[
                    ft.Radio(value=3, label="3 Passes"),
        ...
        self.board_scale = ft.Slider(
            min=0.3, max=1.5, divisions=12, 
            value=self.settings.board_scale, label="{value}x"
        )

        self.content = ft.Column(
            [
                ft.Text("Board Scale (For Mobile):", weight=ft.FontWeight.BOLD),
                self.board_scale,
                ...
                ft.Row(self.card_backs, scroll=ft.ScrollMode.AUTO, height=120),
                ...
                ft.Checkbox(
                    label="Settings restart the game.",
                    value=True,
                    disabled=True,
                ),
            ],
        ...
        self.actions = [
            ...
            ft.FilledButton("Apply", on_click=self.apply_settings),
        ]

    def generate_card_backs(self):
        self.card_backs = []
        for i in range(4):
            img_path = f"/images/card_back{i}.png"
            is_selected = self.settings.card_back == img_path
    ...
    def apply_settings(self, e):
        ...
        self.settings.board_scale = self.board_scale.value
